...
import ComparisonModalPro from "../components/ComparisonModal";
...
// UseDebounce para optimizar la experiencia de búsqueda y evitar cálculos innecesarios
...
export default function ServiceDashboard() {
  ...
  // Para el comparador de servicios <ComparisonModal />
  ...
  const scoredServices = useMemo<ScoredService[]>(() => {
    return services.map((svc) => {
      const result = calculateServiceScore(svc, debouncedFilters);
  ...
  }, [services, debouncedFilters]);

  // Filtrar y Ordenar los servicios por relevancia1111
  const filteredServices = useMemo<ScoredService[]>(() => {
  ...
  const bestMatch = useMemo<ScoredService | null>(() => {
    let best: ScoredService | null = null;

    for (const svc of scoredServices) {
      if (!best || svc.relevanceScore > best.relevanceScore) {
        best = svc;
      }
    }

    return best && best.relevanceScore > 0 ? best : null;
  }, [scoredServices]);

  // Muestra el mejor match por Id
  ...
  return (
    <div className="max-w-7xl mx-auto px-6 py-20 space-y-16">
      ...
      <div className="space-y-8">
        ...
        {bestMatch && <FeaturedRecommendation service={bestMatch} />}
      ...
      <AnimatePresence>
        {compareIds.length === 2 && (
          <ComparisonModalPro
            ...
            services={scoredServices.map((s) => ({
              id: s.id,
              title: s.title,
              impact: s.recommendationMeta.impact,
              effort: s.recommendationMeta.effort,
            }))}
          />
        )}
      </AnimatePresence>
